...
class Item(BaseModel):
    _id: Optional[str] = None # _id is generated by MongoDB, so it's optional on creation
    codigo: str
    nombre: str
    descripcion: str
    ...
    precio: float
    costo: float
    costoProduccion: float = 0.0 # Default to 0.0 if not provided
    cantidad: int
    existencia: int = 0 # New field for stock
    activo: bool = True
    imagenes: Optional[List[str]] = []

class InventarioExcelItem(BaseModel):
    codigo: str
    descripcion: str
    departamento: Optional[str] = None
    marca: Optional[str] = None
